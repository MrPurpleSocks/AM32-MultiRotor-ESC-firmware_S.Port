...
uint8_t aTxBuffer[3];
uint16_t wowSuchIDs[3] = {0x0200, 0x0600, 0x0500};
...
void sendResponse = () {
  ...
  uint8_t buffer = USART1->RDR; // read data

  // check if polling request is for esc
  if (buffer == 0x01) {
    uint32_t response[4] = {0x10, wowSuchIDs[currentValue], aTxBuffer[currentValue], calculateCheksum(aTxBuffer, 3)};
    if (currentValue == 2) {
      currentValue = 0;
    } else {
      currentValue++;
